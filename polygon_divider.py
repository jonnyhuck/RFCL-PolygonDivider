...
from qgis.core import QgsVectorLayer, QgsProject, QgsMessageLog, Qgis, QgsField, QgsFeature, QgsRectangle, QgsGeometry, QgsVectorFileWriter, QgsWkbTypes, QgsDataSourceUri, QgsPoint, QgsCredentials
...
# from polygon_divider_dialog import PolygonDividerDialog
import os.path, os, traceback, sys
    ...
    * Core worker thread - created by Exegesis SDM to process in batches
    ...
    def createTable(self, layer, fieldList, temp = False):
        ...
        try:
            ...
            self.dbConn.close()    
            QgsMessageLog.logMessage("Could not create PostGIS table. {0} Command text: {1}".format(e, createCmd), 'PolygonDivider', level=Qgis.Critical)
    ...
    def dropTempTable(self):
        try:
            ...
            self.dbConn.close()    
            QgsMessageLog.logMessage("Could not delete temporary PostGIS table. {0}".format(e), 'PolygonDivider', level=Qgis.Critical)
    ...
    def writeTempFeature(self, feat, noFields):
        ...
        try:
            ...
            QgsMessageLog.logMessage("Feature could not be written to the temp table. {0} Command text: {1}".format(e, insertCmd), 'PolygonDivider', level=Qgis.Critical)
    #--------------------------------------------------------------------------------- CL #
    ...
    def work(self):
        ...
        * Actually do the processing
        ...
        if self.outputType == 'PostGIS':
            ...
            for ext in ['.shp','.shx','.prj','.dbf','.qpj','.cpg']:
                ...
                if os.path.exists(fileName):
                    try:
                        ...
                        QgsMessageLog.logMessage("{0} could not be deleted. {1}".format(fileName, e), 'PolygonDivider', level=Qgis.Critical)
            ...
            error = QgsVectorFileWriter.writeAsVectorFormatV2(tmpLayer, self.outFilePath, transform_context, save_options)
            if error[0] != QgsVectorFileWriter.NoError:
                QgsMessageLog.logMessage("{0} could not be written. {1}".format(self.outFilePath, error[0]), 'PolygonDivider', level=Qgis.Critical)
                raise Exception("{0} could not be written. {1}".format(self.outFilePath, error[0]))
        ...
        if self.chunk_size < 1:
            QgsMessageLog.logMessage("Chunk size invalid, defaulting to 50", 'PolygonDivider', level=Qgis.Info)
            self.chunk_size = 50
        ...
        if tmpLayer.dataProvider().name() == 'ogr':
            ...
            if tmpLayer.fields()[self.layer.fields().indexFromName(key_field)].type() != 2:
...
class ExampleWorker():
    ...
    def writeFeature(self, feat):
        ...
        try:
            ...
            self.dbConn.rollback()
            QgsMessageLog.logMessage("Feature could not be written to the output table. Reverted latest batch of PostGIS changes. {0} Command text: {1}".format(e, insertCmd), 'PolygonDivider', level=Qgis.Critical)
    ...
    def work(self):
        ...
        * Actually do the processing
                    ...
                    while poly.area() > targetArea + t:
                        ...
                        if (interval[1]-interval[0]) > sq: 
                            ...
                            if ERROR_FLAG_0:
                                ...
                                # log message
                                QgsMessageLog.logMessage("Increasing number of subdivisions didn't work, try decreasing... (Division)", 'PolygonDivider', level=Qgis.Warning)
                            ...
                            if ERROR_FLAG_1:
                                ...
                                # log message
                                QgsMessageLog.logMessage("Decreasing number of subdivisions didn't work, try playing with direction... (Division)", 'PolygonDivider', level=Qgis.Warning)
                                ...
                                if ERROR_FLAG_2 == False:
                                    ...
                                    ERROR_FLAG_2 = True
                                    QgsMessageLog.logMessage("Reversing movement direction (Division)", 'PolygonDivider', level=Qgis.Warning)
                                ...
                                # if the above didn't work, switch the direction of the cutline
                                    ...
                                    # log that this has been tried
                                    ERROR_FLAG_3 = True
                                    QgsMessageLog.logMessage("Reversing cutline direction (Division)", 'PolygonDivider', level=Qgis.Warning)
                                ...
                                # if none of the above worked, just skip it and move to a new feature
                                    ...
                                    QgsMessageLog.logMessage(f"progress {j} {totalDivisions} {currProgress}", 'PolygonDivider', level=Qgis.Info)
                                    ...
                                    QgsMessageLog.logMessage("There was an un-dividable polygon in this dataset.", 'PolygonDivider', level=Qgis.Warning)
                        ...
                        for k in range(nSubdivisions-1):    # nCuts = nPieces - 1
                            ...
                            while True:
                                ...
                                try:
                                    ...
                                    if e.value == "Bracket is smaller than tolerance.":
                                        #QgsMessageLog.logMessage(e.value + ": increasing tolerance (Subdivision)", 'PolygonDivider', level=Qgis.Warning)
                            ...
                            if ERROR_FLAG_1 and not ERROR_FLAG_2: # (NB: Subdivision does not use Errorflag 0)
                                ...
                                # log that this has been tried
                                ERROR_FLAG_2 = True
                                QgsMessageLog.logMessage("Reversing movement direction (Subdivision)", 'PolygonDivider', level=Qgis.Warning)
        ...
        #                                                forward_flag = not forward_flag
                                ...
                                # log that this has been tried
                                ERROR_FLAG_3 = True
                                QgsMessageLog.logMessage("Reversing cutline direction (Subdivision):", 'PolygonDivider', level=Qgis.Warning)
        ...
        #                                            forward_flag = not forward_flag
                ...
                QgsMessageLog.logMessage("Whoops! That dataset isn't polygons!", 'PolygonDivider', level=Qgis.Critical)
...
def worker_error(e, exception_string, iface):
    ...
    QgsMessageLog.logMessage('Worker thread raised an exception: %s' % exception_string, 'PolygonDivider', level=Qgis.Critical)
...
class PolygonDivider:
    ...
    def initGui(self):
        ...
        # TBC Create the dialog (after translation) and keep reference
        self.dlg = PolygonDividerDialog()

        # TBC launch file browser for output file button - link to function
    ...
    def select_output_file(self):
        ...
        * JJH: Open file browser
        ...
        if filename[0] != "":
            ...
            if filename_0[-4:] != '.shp':
                filename_0 = '{0}{1}'.format(filename_0,'.shp')
                # filename = filename + '.shp'

            # put the result in the text box on the dialog
            self.dlg.outputFile.setText(filename_0)
    ...
    def run(self):
        ...
        # TBC maybe only one of the dialog commands below, either orig self.first_start or dlg
        ...
        if result:
            ...
            if self.dlg.rbPostgreSQL.isChecked():
                ...
                if pgDetails['table'] == '':
                    QgsMessageLog.logMessage("PostgreSQL connection or table not specified.", 'PolygonDivider', level=Qgis.Critical)
            ...
            if splitDistance <= (4 * sqrt(targetArea)):
                QgsMessageLog.logMessage("Split Distance must be greater than the 4 x square root of Target Area.", 'PolygonDivider', level=Qgis.Critical)
